...
while True:
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1:
                ...
                if button_run.press:
                    if not src_exist:
                        screen.fill('#FFFFFF',rect=pygame.Rect((1381,720),(320,60)))
                        display_text(screen,"NO SOURCE!",gui_font,'#FF0000',(1490,720),100,100)
                    elif des_exist:
                        try:
                            index_src = get_index_point(screen,x_src,y_src,x_coor,y_coor,min_x,min_y,den_x,den_y)
                            index_des = get_index_point(screen,x_des,y_des,x_coor,y_coor,min_x,min_y,den_x,den_y)
                            start = time.time()
                            ...
                            screen.fill('#FFFFFF',rect=pygame.Rect((1381,720),(320,60)))
                            display_text(screen,"Can't find path, try again!",gui_font,'#FF0000',(1490,720),100,100)
                    else:
                        screen.fill('#FFFFFF',rect=pygame.Rect((1381,720),(320,60)))
                        display_text(screen,"NO DESTINATION!",gui_font,'#FF0000',(1490,720),100,100)
                #Restart button
                if button_res.press:
                    image = pygame.image.load(image_dir)
                    src_exist = False
    ...
    if graph == 0:
        display_text(screen,"LOADING...",gui_font,'#000000', (1535,475),20,20)
        G = load_graph_gpickle(graph_dir + '\Graph_car.gpickle')
        x_coor,y_coor = get_coordinate(data_dir + '\HCM_data_car.pycgr')
        screen.fill('#FFFFFF',rect=pygame.Rect((1381,475),(320,20)))
        car = True
    elif graph == 1:
        display_text(screen,"LOADING...",gui_font,'#000000',(1535,475),20,20)
